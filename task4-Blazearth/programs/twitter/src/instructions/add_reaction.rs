...
pub fn add_reaction(ctx: Context<AddReactionContext>, reaction: ReactionType) -> Result<()> {
    ...
    match reaction{
        ReactionType::Like => tweet_account.likes=tweet_account.likes.checked_add(1)
                                .ok_or(TwitterError::MaxLikesReached)?,
        ReactionType::Dislike => tweet_account.dislikes=tweet_account.dislikes.checked_add(1)
