...
impl Calculator {
    ...
    pub fn addition(&mut self, x: i64, y: i64) -> Option<i64> {
        let res = OperationType::Addition.perform(x, y);
    if let Some(_) = res {
    ...
    pub fn subtraction(&mut self, x: i64, y: i64) -> Option<i64> {
        let res = OperationType::Subtraction.perform(x, y);
        if let Some(_) = res {
    ...
    pub fn multiplication(&mut self, x: i64, y: i64) -> Option<i64> {
        let res = OperationType::Multiplication.perform(x, y);
        if let Some(_) = res {
    ...
    pub fn show_history(&self) -> String {
        ...
        for (i, operation) in self.history.iter().enumerate() {
            ...
            if let Some(value) = result {
                history_string.push_str(&format!("{}: {} {} {} = {}\n", i, operation.first_num, operation.operation_type.get_sign(), operation.second_num, value));
            }
        }
        history_string
    }
